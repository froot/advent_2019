...
def trace_path(wire)
  ...
  wire.split(',').each do |ins|
    case ins[0]
    when "U"
      ins[0]=""
      (1..ins.to_i).each do |int|
        steps+=1
        path1 << [x, y + int, steps]
      end
      y = y + ins.to_i
...
path1 = trace_path(wire_1)
path2 = trace_path(wire_2)

intersections = []

hash = path1.each_with_object({}) do |coord1, hash|
  hash[[coord1[0], coord1[1]]] = coord1[2]
end

path2.each do |coord|
  if hash[[coord[0], coord[1]]]
    first_steps = hash[[coord[0], coord[1]]]
    second_steps = coord[2]
