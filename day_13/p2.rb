...
class Amp
  ...
  def initialize
    @program = File.read('input2.txt').chomp.split(',').map(&:to_i)
    @current_index = 0
    @waiting = true
    @relative_base = 0
    @output = []
  ...
  def execute_3(params)
    ...
    @program[position_of_param(params.last)] = 0
    @waiting = true
...
def print_screen(tiles)
  screen = tiles.each_slice(3).map do |slice|
    [[slice.first, slice.fetch(1)], slice.last]
  end.to_h

  x_max = screen.keys.map(&:first).max
  y_max = screen.keys.map(&:last).max
